...
const Sidebar = () => {
  ...
  useEffect(() => {
    if (socketConnection) {
      ...
      socketConnection.on("send message", (data) => {
        if (data?.receiver === user?._id) {
          console.log("@@@@@@firingup send message", data);
    ...
    //TODO:cleanup funtion
    return () => {
      if (socketConnection) {
        socketConnection.off("send message");
  ...
  return (
    <div className="w-full h-full grid grid-cols-[48px,1fr] bg-white">
      <div className="bg-slate-100 w-12 h-full rounded-tr-lg rounded-br-lg py-5 text-slate-600 flex flex-col justify-between">
        <div>
          ...
          <div
            ...
            onClick={() => setScheduleMessageOpen(true)}
            className="w-12 h-12 flex justify-center items-center cursor-pointer hover:bg-slate-200 rounded"
          >
      ...
      {scheduleMessageOpen && (
        <ScheduleMessage onClose={() => setScheduleMessageOpen(false)} />
      )}
    </div>
  );
};
